package com.phoenix.devops.cache;

import com.phoenix.devops.utils.ScheduleUtil;
...
public class TimedCache<K, V> extends AbstractCache<K, V> {
    ...
    protected void putWithoutCheck(K key, V object) {
        ...
        if (!ScheduleUtil.containSchedule(getScheduleId())) {
    ...
    public void remove(K key) {
        ...
        if (cacheMap.isEmpty()) {
            ScheduleUtil.cancel(getScheduleId());
        }
    }
    ...
    private void scheduleRemoveKey() {
        ...
        ScheduleUtil.scheduleAtFixedRate(getScheduleId(), () -> {
